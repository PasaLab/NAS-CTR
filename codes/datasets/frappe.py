...
class FrappeDataset(Dataset):
    ...
    def __init__(self, data_path, repreprocess=False, use_pretrained_embeddings=True, logger=None):
        ...
        self._num_sparse_feats = 10
        super(FrappeDataset, self).__init__(data_path=data_path,
    ...
    def _get_feat_dict(self):
        ...
        with open(self._data_path) as f:
            ...
            pbar.set_description('Count feats of Frappe dataset.')
            for line in pbar:
                values = line.rstrip('\n').split(',')
    ...
    def _transform_feat(self, feat_dict, defaults):
        ...
        with open(self._data_path) as f:
            ...
            if self._with_head:
                f.readline()
            pbar = tqdm(f, mininterval=1, smoothing=0.1)
            pbar.set_description('Transform feats of Frappe dataset.')
            for line in pbar:
                values = line.rstrip('\n').split(',')
                if len(values) != self._num_feats + 1:
                    continue
    ...
    def _read(self):
        if self._repreprocess:
            self._logger.info("Start to preprocess Frappe dataset")
        ...
        super(FrappeDataset, self)._read()


if __name__ == '__main__':
    data_path = f'{PROJECT_PATH}/codes/datasets/data/frappe.txt'
    logger = get_logger('frappe', 'read_frappe.log')
    ds = FrappeDataset(data_path, repreprocess=True, logger=logger)
