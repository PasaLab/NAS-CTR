...
class MovielensDataset(Dataset):
    ...
    def __init__(self, data_path, repreprocess=False, use_pretrained_embeddings=True, logger=None):
        ...
        self._num_sparse_feats = 3
        super(MovielensDataset, self).__init__(data_path=data_path,
    ...
    def _get_feat_dict(self):
        ...
        with open(self._data_path) as f:
            ...
            pbar.set_description('Count feats of Movielens dataset.')
            for line in pbar:
                values = line.rstrip('\n').split(',')
    ...
    def _transform_feat(self, feat_dict, defaults):
        ...
        with open(self._data_path) as f:
            ...
            if self._with_head:
                f.readline()
            pbar = tqdm(f, mininterval=1, smoothing=0.1)
            pbar.set_description('Transform feats of Movielens dataset.')
            for line in pbar:
                values = line.rstrip('\n').split(',')
                if len(values) != self._num_feats + 1:
                    continue
    ...
    def _read(self):
        if self._repreprocess:
            self._logger.info("Start to preprocess Movielens dataset")
            ...
            self._split_dataset()
            self._logger.info("Finish preprocess Movielens dataset")
        super(MovielensDataset, self)._read()


if __name__ == '__main__':
    data_path = f'{PROJECT_PATH}/codes/datasets/data/movielens.txt'
    logger = get_logger('movielens', 'read_movielens.log')
    ds = MovielensDataset(data_path, repreprocess=True, logger=logger)
